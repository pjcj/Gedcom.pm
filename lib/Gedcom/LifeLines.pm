# Copyright 1999-2024, Paul Johnson (paul@pjcj.net)
...
use Exporter;
BEGIN {  # We'll use these if they are available
  eval "use Date::Manip";
  eval "use Roman ()";
}
...
sub fullname {
    ...
    my $name = $inorder
        ? name($indi, $cased)
        : ($cased ? uc $indi->surname : $indi->surname) . ", "
                                                        . $indi->given_names;
...
sub pn {
    my ($indi, $type) = @_;
    return unless $indi;
    (qw(He She he she His Her his her him her))[
        $type * 2 + ($indi->sex eq "F" ? 1 : 0)
    ]
}
...
sub getindimsg { getindi(@_) }

sub getindiset { die "LifeLines getindiset function not yet implemented" }
...
sub getstrmsg { getstr(@_) }

sub choosechild  { die "LifeLines choosechild function not yet implemented"  }
sub choosefam    { die "LifeLines choosefam function not yet implemented"    }
sub chooseindi   { die "LifeLines chooseindi function not yet implemented"   }
sub choosesubset { die "LifeLines choosesubset function not yet implemented" }
sub menuchoose   { die "LifeLines menuchoose function not yet implemented"   }
...
sub gengedcom  { die "LifeLines gengedcom function not yet implemented"  }
sub createnode { die "LifeLines createnode function not yet implemented" }
sub addnode    { die "LifeLines addnode function not yet implemented"    }
sub deletenode { die "LifeLines deletenode function not yet implemented" }
...
sub getrecord { dereference(@_) }
sub lock      { return          }
sub unlock    { return          }
sub database  { $Ged->{file}    }
sub version   { $VERSION        }
sub system    { system(@_)      }
